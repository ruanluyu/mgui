...
namespace mgui{
	...
	namespace geo {
		...
		struct Vector2D {
			dataT x, y;
			Vector2D() :x(), y() {}
			Vector2D(dataT _0, dataT _1) : x(y), x(y) {}
		};
		...
		using Size2Dpx = Size2D<pixel_loc_type>;
		using Size2Dpt = Size2D<point_loc_type>;

		using Position2Dpx = Position2D<pixel_loc_type>;
		using Position2Dpt = Position2D<point_loc_type>;
		...
		struct Rectangle2D
		{
			...
			inline static T min(T l, T r) {
				return l < r ? l : r;
			}

			inline static T max(T l, T r) {
				return l > r ? l : r;
			}
			...
			static Rectangle2D<T> FromMinMax(pointT minP, pointT maxP) {
				return Rectangle2D<T>(minP, pointT(maxP.x - minP.x, maxP.y - minP.y));
			}
		...
		using Rectangle2Dpx = Rectangle2D<pixel_loc_type>;
		using Rectangle2Dpt = Rectangle2D<point_loc_type>;
